...
public class Transaction {
    ...
    /**
     * ADDITIONAL FUNCTIONALITY Returns the boolean value stored at the
     * specified offset of the specified block. The method first obtains an
     * SLock on the block, then it calls the buffer to retrieve the value.
     *
     * @param blk a reference to a disk block
     * @param offset the byte offset within the block
     * @return the boolean stored at that offset
     */
    public boolean getBoolean( Block blk, int position ) {
        System.out.println( "tx.Transaction says : getBoolean( Block : ..., position : " + position + " )" );
        concurMgr.sLock( blk );
        Buffer buff = myBuffers.getBuffer( blk );
        return buff.getBoolean( position );
    }
    ...
    /**
     * ADDITIONAL FUNCTIONALITY Stores a boolean at the specified offset of the
     * specified block. The method first obtains an XLock on the block. It then
     * reads the current value at that offset, puts it into an update log
     * record, and writes that record to the log. Finally, it calls the buffer
     * to store the value, passing in the LSN of the log record and the
     * transaction's id.
     *
     * @param blk a reference to the disk block
     * @param offset a byte offset within that block
     * @param val the value to be stored
     */
    public void setBoolean( Block blk, int position, boolean val ) {
        System.out.println( "tx.Transaction says : setBoolean( Block .... , int : " + position + ", bool :" + val + " )" );
        concurMgr.xLock( blk );
        Buffer buff = myBuffers.getBuffer( blk );
        int lsn = recoveryMgr.setBoolean( buff, position, val );
        buff.setBoolean(position, val, txnum, lsn );
    }
