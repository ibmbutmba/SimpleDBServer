...
public class RecoveryMgr {
    ...
    public int setBoolean( Buffer buff, int position, boolean val ) {
        System.out.println( "recovery.RecoveryMgr says : setBoolean( Buffer .... , int : " + position + ", bool :" + val + " )" );
        boolean oldval = buff.getBoolean( position );
        Block blk = buff.block();
        if ( isTempBlock( blk ) ) {
            return -1;
        } else {
            return new SetBooleanRecord( txnum, blk, position, oldval ).writeToLog();
        }
    }
