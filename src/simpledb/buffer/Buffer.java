...
public class Buffer {
     ...
     * {@link BasicBufferMgr}. It depends on the
    ...
    /**
     * Returns the boolean value at the specified offset of the buffer's page.
     * If a boolean was not stored at that location, the behavior of the method
     * is unpredictable.
     *
     * @param offset the byte offset of the page
    ...
    public boolean getBoolean( int position ) {
        System.out.println( "buffer.Buffer says : getBoolean( position : " + position + " )" );
        return contents.getBoolean( position );
    }
    ...
    /**
     * Writes a boolean to the specified offset of the buffer's page. This method
     * assumes that the transaction has already written an appropriate log
     * record. A negative lsn value indicates that a log record was not
     * necessary. The buffer saves the id of the transaction and the LSN of the
     * log record.
     *
     * @param offset the byte offset within the page
     * @param val the new boolean value to be written
     * @param txnum the id of the transaction performing the modification
     * @param lsn the LSN of the corresponding log record
     */
    public void setBoolean( int offset, boolean val, int txnum, int i ) {
    System.out.println( "buffer.Buffer says : setBoolean( int offset "+ offset+ ", bool : ... )" );
        
    modifiedBy = txnum;
        if ( i >= 0 ) {
            logSequenceNumber = i;
        }
        contents.setBoolean( offset, val );
    }
