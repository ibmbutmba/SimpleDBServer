...
public class NoDupsHashPlan implements Plan {
    ...
    private List<TempTable> splitIntoRunsHashing( Scan scan1 ) {
        ...
        for ( int j = 0; j < fields.size(); j++ ) {
            while ( i < 5 && scan.getString( fields.get( j ) ).length() > i ) {
                value += hmap.get( ("" + (scan.getString( fields.get( j ) ).charAt( i ))).toLowerCase() );
                i++;
                if ( i == 5 ) {
                    addtoTemp( value, scan, temp1Values, temp2Values, fields.get( j ) );
    ...
    private boolean addtoTemp( int value, UpdateScan scan, ArrayList<Constant> temp1Values, ArrayList<Constant> temp2Values, String currFieldName ) {
        boolean existing = false;
        if ( value < 65 ) {
            for ( Constant cons : temp1Values ) {
                if ( scan.getVal( currFieldName ).equals( cons ) ) {
                    existing = true;
                }
            }
            if ( !existing ) {
                temp1Values.add( scan.getVal( currFieldName ) );
            }
            existing = false;
        } else {
            ...
            if ( !existing ) {
                temp2Values.add( scan.getVal( currFieldName ) );
            }
            existing = false;
        }
        return existing;
    ...
    private void setValues( TempTable temp1, ArrayList<Constant> namesList ) {
        ...
        for ( Constant actualScan : namesList ) {
            ...
            for ( int i = 0; i < fields.size(); i++ ) {
                ...
                updateScan1.setVal( fields.get( i ), actualScan );
            }
        }
        updateScan1.close();
    }
